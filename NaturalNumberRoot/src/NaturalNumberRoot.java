...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        NaturalNumber remainder = new NaturalNumber2(lowEnough.divide(two));
        NaturalNumber middle = new NaturalNumber2(lowEnough);
        if (remainder.compareTo(one) == 0) {
            lowEnough.multiply(two);
            lowEnough.add(one);
            lowEnough.subtract(tooHigh);
        } else {
            lowEnough.multiply(two);
            lowEnough.subtract(tooHigh);
        }
        ...
        while (difference.compareTo(one) > 0) {
            ...
            middleCopy.power(r);
            if (middleCopy.compareTo(n) > 0) {
                tooHigh.copyFrom(middle);
                lowEnough.add(tooHigh);
                remainder = lowEnough.divide(two);
                middle.copyFrom(lowEnough);
                if (remainder.compareTo(one) == 0) {
                    lowEnough.multiply(two);
                    lowEnough.add(one);
                    lowEnough.subtract(tooHigh);
