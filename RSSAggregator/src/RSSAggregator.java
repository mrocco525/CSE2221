...
/**
 * Program that aggregates multiple RSS 2.0 feeds into one HTML document given a
 * link to an XMLTree containing RSS 2.0 feeds.
 *
 * @author Mason Rocco
 *
 */
public final class RSSAggregator {
    ...
    /**
     * Outputs the header of the main HTML document.
     *
     * @param xml
     *            XMLTree with RSS 2.0 feed links
     * @param out
     *            output stream
     */
    private static void outputMainHeader(XMLTree xml, SimpleWriter out) {
        assert xml != null : "Violation of: channel is not null";
        assert out != null : "Violation of: out is not null";
        assert out.isOpen() : "Violation of: out.is_open";
        //starts the html document and prints out the title
        out.println("<html> <head> <title>");
        out.println(xml.attributeValue("title") + "</title>");
        out.println("</head>");
        //starts the body of the document and creates a header
        out.println("<body>");
        out.println("<h2>" + xml.attributeValue("title") + "</h2>");
        //creates a list that will contain links to all of the RSS feeds
        out.println("<ul>");
        int x = 0;
        while (x < xml.numberOfChildren()) {
            String file = xml.child(x).attributeValue("file");
            String name = xml.child(x).attributeValue("name");
            out.println("<li><a href=\"" + file + "\">" + name + "</a></li>");
            x++;
        }
        //ends the list, the body, and the html document
        out.println("</ul>");
        out.println("</body>");
        out.println("</html>");
    }
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        while (label != "rss" && version != "2.0") {
            out.println("The link inputted is not a valid RSS 2.0 news feed.");

            out.println("Enter another link of a valid RSS 2.0 feed:");
            XMLTree newTree = new XMLTree1(url);

            label = newTree.label();
            if (newTree.hasAttribute("version")) {
                version = newTree.attributeValue("version");
            }
        }
        ...
        //outputs the beginning of the HTML document
        ...
        if (getChildElement(channel, "description") == -1) {
            String descriptionDetail = description.child(0).label();
            outputFile.println("<p>" + descriptionDetail + "</p>");
        }
        ...
        //outputs the ending of the HTML document
    ...
    public static void main(String[] args) {
        ...
        //gets XML File with list of feeds
        out.print("Enter an XML File with RSS 2.0 news feeds: ");
        String url = in.nextLine();
        XMLTree xml = new XMLTree1(url);

        //gets output file name and creates a SimpleWriter for it
        out.print("Enter the name for an output file: ");
        String file = in.nextLine();
        SimpleWriter html = new SimpleWriter1L(file);
        outputMainHeader(xml, html);

        //creates all of the RSS feeds
        int x = 0;
        while (x < xml.numberOfChildren()) {
            String fileName = xml.child(x).attributeValue("file");
            processFeed(xml.child(x).attributeValue("url"), fileName, html);
            x++;
        }
