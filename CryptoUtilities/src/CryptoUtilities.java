...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (copyP.isZero()) {
            // if p is zero, then n^p = 1
            n.copyFrom(one);
        } else if (isEven(copyP)) {
            // if p is even, divide p by 2 to use fast recursive power
            copyP.divide(two);
            powerMod(n, copyP, m);
            NaturalNumber copyN = new NaturalNumber2(n);
            n.multiply(copyN);
            ...
            // if p is odd, divide p by 2 and multiply by original n
            copyP.divide(two);
            powerMod(n, copyP, m);
            NaturalNumber copyN = new NaturalNumber2(n);
            n.multiply(copyN);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        return (copyW.equals(one) || !w.equals(one));
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            // if n is less than 3, it is prime
            isPrime = true;
        } else if (isEven(n)) {
            // if n is even, it cannot be prime
            isPrime = false;
        } else {
            // generates 100 random numbers less than (n-1)
            for (int i = 1; i < hundred; i++) {
                NaturalNumber guess = randomNumber(copyN);
                while (guess.compareTo(one) <= 0
                        || guess.compareTo(copyN) >= 0) {
                    guess = randomNumber(copyN);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // if n is even, add one to make it odd (no even prime numbers)
        if (isEven(n)) {
            n.increment();
        }

        // make n the next odd number until n is a prime number
        while (!isPrime2(n)) {
            n.increment();
            n.increment();
        }
