...
public final class PizzaOrderManager {
        ...
        while (!in.atEOS()) {
            ...
            for (int i = 0; i < s.length(); i++) {
                if (s.charAt(i) == ',') {
                    ...
                    price = s.substring(i + 1);
                }
            }
            int price2 = Integer.parseInt(price);
            priceMap.add(item, price2);
        }
        in.close();
    }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        int price = 0;
        price = price + sizePriceMap.value(input.nextLine());
        while (!input.atEOS()) {
            String topping = input.nextLine();
            if (toppingPriceMap.hasKey(topping)) {
                price = price + toppingPriceMap.value(topping);
            }
        }
        return price;
    }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        String dollar = money.substring(0, money.length() - 2);
        String cents = money.substring(money.length() - 2, money.length());
        output.print(dollar + " dollars and " + cents + " cents");
    }
